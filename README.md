...
This runs `node --test js/points.test.js js/main.test.js js/events.test.js`. Harnesses in `js/points.harness.js`, `js/main.harness.js`, and `js/events.harness.js` load each script in a mocked browser environment so scoring logic and CMS rendering can be tested without opening the page.
...
3. Open the repo — the `.pages.yml` config defines all editable content:
   ...
   - **Events Page** — Listing page heading and subheading
   - **Events** — Individual event entries (flyer, date, price, location, description, registration link)

Changes save directly to the JSON files in `data/` and deploy with your next push.

### Events workflow

Each event lives in `data/events/` as its own JSON file. The URL slug is autogenerated as `{YYYY-MM-DD}-{title-slug}` (for example, `/events/2026-08-15-summer-mahjong-night/`).

After adding or editing events in Pages CMS, run:
...
- `data/events-index.json` — used by the `/events/` listing page
- `events/{slug}/index.html` — detail page shells with per-event SEO meta
- Event URLs in `sitemap.xml`

Commit the generated files along with your CMS changes. `npm test` runs the sync step automatically first.
...
│   ├── _detail/index.html  # Detail page template (used by sync script)
│   └── {slug}/index.html   # Generated per-event detail pages
├── hong-kong-mahjong-scoring/
│   └── index.html        # Scoring guide + interactive points calculator
├── scripts/
│   └── sync-events.js    # Generates event index, detail routes, and sitemap entries
├── logo.png            # Club logo
├── css/
│   ├── styles.css        # Shared styles (CSS variables from styleguide)
│   ├── events.css        # Styles for the events pages
...
│   ├── events.js         # Events listing + detail page renderer
│   ├── events-lib.js     # Shared slug/date helpers (browser + Node)
│   ├── events.harness.js # Test harness for events utilities
...
│   ├── events-page.json  # Events listing page copy
│   ├── events-index.json # Generated event index for the listing page
│   └── events/           # Per-event CMS JSON files
